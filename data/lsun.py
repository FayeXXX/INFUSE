...
import numpy as np
from config import lsuncrop_root, lsunresize_root
...
class CustomLSUN(LSUN):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(CustomLSUN, self).__init__(*args, **kwargs)
...
def get_lsun_crop_datasets(train_transform, test_transform, train_classes=range(4),
    ...
    train_dataset_whole = CustomLSUN(root=lsuncrop_root, transform=train_transform, train=True)
    ...
    test_dataset_known = CustomLSUN(root=lsuncrop_root, transform=test_transform, train=False)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    test_dataset_unknown = CustomLSUN(root=lsuncrop_root, transform=test_transform, train=False)
...
if __name__ == '__main__':
    ...
    x = get_lsun_crop_datasets(None, None, split_train_val=False, balance_open_set_eval=False)
